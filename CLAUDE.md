...
- **~~Single-Instance~~ → wird ersetzt durch Hub + typisierte Subentries** (Entscheidung 2026-05-29). Die flache Single-Config wurde zur „Litanei". Zielbild: **Hub-Entry** (Foundation-Entities EINMAL + auto-prefill aus Toolbox, globale Gates, Arbitrierung, Apply) + **ConfigSubentries pro Anwendungsfall** (typisiert: Dayphase, Gaming, Musik, Notification-RGB, Flur, Bad, Schlafzimmer). Migration in 3 Phasen, je PR. **Phase 1 ✅:** §4.1 ist eine typisierte Policy-Registry (`policy.LIVING_ROOM_POLICIES`), verhaltenstreu. Phase 2: Subentries + Auto-Prefill. Phase 3: Notification/Bereiche als Subentry-Typen.
