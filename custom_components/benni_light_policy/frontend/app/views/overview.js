...
export function render(el, ctx) {
  ...
  const keys = store.policyKeys();
  const sum = store.coverageSummary(keys);
  ...
  const summaryRows = [
    ...
    ["Lux-Gate", gate.lux_gate_on ? "offen (dunkel genug)" : "zu (zu hell)", gate.lux_gate_on ? "ok" : "info"],
  ...
  el.innerHTML = `
      ...
      <div class="card" style="grid-column: span 2;">
        ...
        <div class="grid cols-3">
          <div class="tile">
            ...
            <div class="subtext">${esc(desiredKey || "idle")}</div>
            <div style="margin-top:8px">${cov ? coverageChip(cov) : chip("info", "kein Look")}</div>
          </div>
          <div class="tile">
            <div class="lbl">Tatsächlich laufend</div>
            <div class="big">${desiredLook ? esc(desiredLook.name || desiredLook.slug) : "—"}</div>
            <div style="margin-top:8px">${
              running == null ? chip("info", "unbekannt")
                : chip(running === "on" ? "ok" : "info", running === "on" ? "läuft" : "aus")}</div>
          </div>
          <div class="tile">
            <div class="lbl">Helligkeit (Tagesphase)</div>
            <div class="big">${briPct != null ? briPct + " %" : "—"}</div>
            <div class="subtext">${plan.brightness != null ? plan.brightness + " / 255" : "kein Override"}</div>
          </div>
          <div class="tile">
            <div class="lbl">Modus</div>
            <div class="big">${esc(plan.mode || "—")}</div>
          </div>
          <div class="tile">
            <div class="lbl">Scene-Hash</div>
            <div class="mono" style="margin-top:6px">${esc(plan.scene_hash || "—")}</div>
          </div>
          <div class="tile">
            <div class="lbl">Änderungsgrund</div>
            <div style="margin-top:4px;font-size:13px">${esc(plan.reason || "—")}</div>
          </div>
        </div>
      </div>
    ...
    <div class="grid cols-3" style="margin-top:14px">
      <div class="card">
        <h2><span class="ico">🛡️</span>Look-Coverage</h2>
        ${donut(sum)}
      ...
      <div class="card" style="grid-column: span 2;">
        ...
        <div class="kv"><span class="k">Lux-Gate<small>aus Wetter/Lux/Tagesphase</small></span>
          <span class="v">${chip(gate.lux_gate_on ? "ok" : "info", gate.lux_gate_on ? "offen" : "zu")}</span></div>
...
function donut(sum) {
  ...
  return `
    <div style="display:flex;gap:16px;align-items:center">
      <div class="donut" style="width:120px;height:120px;border-radius:50%;
        ...
        <div style="width:84px;height:84px;border-radius:50%;background:var(--card);display:grid;place-items:center">
          <div style="text-align:center"><b style="font-size:20px">${sum.total}</b><br><small class="muted">Policy-Keys</small></div>
        </div>
      </div>
      <div class="legend">
        <div><span class="sw" style="background:var(--green)"></span>Vollständig <b>${sum.ok}</b></div>
        <div><span class="sw" style="background:var(--yellow)"></span>Fehlend <b>${sum.missing}</b></div>
        <div><span class="sw" style="background:var(--red)"></span>Ungültig <b>${sum.invalid}</b></div>
      </div>
    </div>`;
}
